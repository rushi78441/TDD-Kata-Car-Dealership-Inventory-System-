...
async def test_should_able_to_search_vehicles_by_query_params(client, admin_auth_headers):
    """
    Test searching vehicles by brand, category, and price range.
    """
    # pre populate 3 different vehicles
    v1 = {"brand": "Toyota", "model": "Corolla", "category": "Sedan", "price": 20000.0, "quantity": 5}
    v2 = {"brand": "Toyota", "model": "RAV4", "category": "SUV", "price": 32000.0, "quantity": 4}
    v3 = {"brand": "BMW", "model": "X5", "category": "SUV", "price": 65000.0, "quantity": 2}

    await client.post("/api/vehicles", json = v1, headers = admin_auth_headers)
    await client.post("/api/vehicles", json = v2, headers = admin_auth_headers)
    await client.post("/api/vehicles", json = v3, headers = admin_auth_headers)

    # Search method 1: Filter by brand = Toyota (should return Corolla & RAV4)
    response1 = await client.get("/api/vehicles/search?brand=Toyota")
...
async def test_should_update_vehicle_successfully(client, admin_auth_headers):
    ...
    # populate a vehicle
    v_payload = {"brand": "Honda", "model": "Civic", "category": "Sedan", "price": 22000.0, "quantity": 5}
    ...
    assert data["brand"]  ==  "Honda"  # Unchanged field remains intact
...
async def test_should_delete_vehicle_successfully(client, admin_auth_headers):
    ...
    # populate a vehicle
    v_payload = {"brand": "Ford", "model": "Focus", "category": "Hatchback", "price": 18000.0, "quantity": 2}
    create_response = await client.post("/api/vehicles", json = v_payload, headers = admin_auth_headers)
    vehicle_id = create_response.json()["id"]
...
async def test_purchase_vehicle_decreases_quantity(client, customer_auth_headers, admin_auth_headers):
    ...
    v_payload = {"brand": "Mazda", "model": "CX-5", "category": "SUV", "price": 28000.0, "quantity": 2}
    create_response = await client.post("/api/vehicles", json = v_payload, headers = admin_auth_headers)
    vehicle_id = create_response.json()["id"]
...
async def test_purchase_out_of_stock_fails(client, customer_auth_headers, admin_auth_headers):
    ...
    v_payload = {"brand": "Tesla", "model": "Model 3", "category": "Sedan", "price": 40000.0, "quantity": 0}
    create_response = await client.post("/api/vehicles", json = v_payload, headers = admin_auth_headers)
    vehicle_id = create_response.json()["id"]
...
@pytest.mark.asyncio
async def test_restock_vehicle_increases_quantity(client, admin_auth_headers):
    """
    Test restocking a vehicle increases its quantity (Admin only).
    """
    
    # Seed a vehicle with quantity = 1
    v_payload = {"brand": "Subaru", "model": "Outback", "category": "Wagon", "price": 31000.0, "quantity": 1}
    create_response = await client.post("/api/vehicles", json = v_payload, headers = admin_auth_headers)
    vehicle_id = create_response.json()["id"]

    # Restock 5 units
    restock_response = await client.post(
        f"/api/vehicles/{vehicle_id}/restock?quantity=5", 
        headers = admin_auth_headers
    )
